# Implement normal equations method from scratch and compare your results on the  admissions dataset
...
def Normal_equation(x, y):
    nor_eq_theta = np.linalg.inv(x.T.dot(x)).dot(x.T.dot(y))
    return nor_eq_theta


def train_model(filepath, target_column, columns_to_drop, alpha=0.01, iterations=1000):
    x, y = load_data(filepath, target_column, columns_to_drop)
    # print(x)
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3,
                                                        random_state=42)  # scikit learn for splitting the data
    ...
    # prediction of y using normal equation theta's
    y_pred_normal = hypo(x_test, normal_theta)
    print(f"Predicted Y from Normal equation: {y_pred_normal}")

    # r2 score computation on Normal equation
    r2score = r2_score(y_test, np.array(y_pred_normal))
    print(f"r2score of Normal equation: {r2score:.2f}")
    ...
    # predict y values based on the x_test using Gradient descent from scratch
    y_pred = hypo(x_test, theta)

    # compute the r2 score
    r2score = r2_score(y_test, np.array(y_pred))
    print(f"r2score of GDA: {r2score:.2f}")

    '''r2 score computed by my GDA implementation for chance of admit  target comes to 0.83,
    r2 score computed using Normal equation 0.83
    r2 score by scikit 0.8206'''
...
if __name__ == '__main__':
    ...
    f_theta = train_model(filepath, target_column, columns_to_drop, alpha=0.01, iterations=10000)
