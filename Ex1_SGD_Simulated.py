...
'''Trained model using scratch implementation for stimulated data set, computed the r2 scores for both disease score and disease_score_fluct'''
...
def train_model(filepath,target_column,columns_to_drop,alpha = 0.01,iterations=1000):
    ...
    for i in range(iterations):
        ...
        x_sample = x_train[indices].reshape(1,-1)
        y_sample = np.array([y_train[indices]])

        #compute hypothesis
        h_xes = hypo(x_sample,theta)
        hxes = np.array(h_xes)

        #compute cost
        cost_f =  cost(hxes, y_sample)

        # load the error
        error = hxes - y_sample

        #compute the gradient
        gd = grad_f(error, x_sample)
...
if __name__ == '__main__':
    ...
    f_theta  = train_model(filepath, target_column,columns_to_drop, alpha=0.0001, iterations=100000)
